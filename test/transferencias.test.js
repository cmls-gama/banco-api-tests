...
const putTrasnferencias = require('../fixtures/putTransferencias.json')
...
describe('Transferencias', () => {
    ...
    describe('PUT/transferencias/{id}', () => {
        it('Deve atualizar o valor da transferencia', async () => {
            ...
            const bodyTransferencias = { ...putTrasnferencias }
            const resposta = await request(process.env.BASE_URL) //request aqui é do SUPERTEST

                .put(`/transferencias/${idTransferencia}`)
                .set('Content-Type', 'application/json')
                .set('Authorization', `Bearer ${token}`)
                .send(bodyTransferencias)
